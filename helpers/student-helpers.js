...
module.exports = {
  verifyNumbert: (userDtails) => {
    return new Promise(async (resolve, reject) => {
      ...
      let user = await db
        ...
        .findOne({ Mobile: userDtails.Mobile });
      if (user) {
  ...
  doLoginEmail: (userDtails) => {
    return new Promise(async (resolve, reject) => {
      let student = await db
        .get()
        .collection(collection.Students_collection)
        .findOne({ Email: userDtails.Email });
      if (student) {
    ...
    return new Promise(async (resolve, reject) => {
      await db
        ...
        .update(
          ...
          { $pull: { Assignment: { _id: objectId(id) } } }
        )
        .then((student) => {
          resolve(student);
        })
  ...
  postAssignment: (studentId, totorId, assinfo, FileAddress, fileName) => {
    ...
    return new Promise(async (resolve, reject) => {
      let topic = await db
        .get()
        .collection(collection.Tutor_collection)
        .findOne({ Assignment: { $elemMatch: { Topic: assinfo.Topic } } });
      if (topic) {
        db.get()
          .collection(collection.Students_collection)
          .updateOne(
      ...
      let topic = await db
        ...
        .findOne({
          ...
          $and: [
            ...
            { Attendance: { $elemMatch: { Attendance: "Absent" } } },
          ],
        });
      if (topic) {
        db.get()
    ...
    return new Promise(async (resolve, reject) => {
      let tod = await db
        ...
        .aggregate([
          {
            $match: {
              "Notes.PostDate": new Date(Date.now())
                .toLocaleString()
                .split(",")[0],
            },
          },
    ...
    return new Promise(async (resolve, reject) => {
      let tod = await db
        ...
        .aggregate([
          {
            $match: {
              "Assignment.PostDate": new Date(Date.now())
                .toLocaleString()
                .split(",")[0],
            },
          },
  ...
  bookEvent: (book, userId) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      db.get()
        .collection(collection.BookedEvent_collection)
        .insertOne(booknow)
        .then((response) => {
          resolve(response);
        })
  ...
  getBooked: (StudenId, event) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collection.BookedEvent_collection)
        .findOne({
          $and: [{ StudentId: objectId(StudenId) }, { eventName: event }],
        })
        .then((user) => {
          resolve(user);
        })
